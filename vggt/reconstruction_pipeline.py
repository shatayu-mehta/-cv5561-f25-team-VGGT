...
COLOR_TOLERANCE = 50    # Threshold to distinguish Object vs Floor
PALETTE_SIZE = 5        # Number of object colors to learn
BG_PALETTE_SIZE = 3     # Number of background colors to learn (and ban)
...
def lift_anchor_to_3d(mask, depth_path, pose_path, intr_path, img_path_for_color=None):
    ...
    Performs Background Learning to purify color palette.
    """
    print(" [2] Lifting Anchor to 3D (Core Sampling & Background Learning)...")
    ...
    # --- 1. CORE OBJECT MASK (Erosion) ---
    kernel = np.ones((5, 5), np.uint8)
    eroded_mask = cv2.erode(mask, kernel, iterations=1)
    if np.sum(eroded_mask) < 100: eroded_mask = mask
    
    # --- 2. BACKGROUND MASK (Ring around object) ---
    # Dilate original mask significantly, then subtract original mask
    kernel_bg = np.ones((20, 20), np.uint8)
    dilated_full = cv2.dilate(mask, kernel_bg, iterations=2)
    bg_ring = cv2.subtract(dilated_full, mask)
    ...
    if img_path_for_color:
        ...
        if img_array is not None:
            # A. Learn Object Colors
            palette = extract_dominant_palette(img_array, eroded_mask, k=PALETTE_SIZE)
            
            # B. Learn Background Colors (The Enemy)
            bg_palette = None
            if np.sum(bg_ring) > 100:
                bg_palette = extract_dominant_palette(img_array, bg_ring, k=BG_PALETTE_SIZE)
            
            # C. PURIFY PALETTE (Delete Object colors that look like Background)
            if palette is not None and bg_palette is not None:
                print("  -> Purifying Palette (Removing colors matching background)...")
                clean_palette = []
                for obj_c in palette:
                    # Find distance to NEAREST background color
                    # obj_c: (3,)  bg_palette: (K, 3)
                    dists = np.linalg.norm(bg_palette - obj_c, axis=1)
                    min_dist_to_bg = np.min(dists)
                    
                    if min_dist_to_bg > COLOR_TOLERANCE:
                        clean_palette.append(obj_c)
                    else:
                        print(f"     [x] Removed ambiguous color: {obj_c} (Too close to floor)")
                
            if len(clean_palette) >= 2:
                palette = np.array(clean_palette)
            else:
                print(f"     [!] Warning: Filter too aggressive ({len(clean_palette)}/{PALETTE_SIZE} remaining). Reverting to unpurified.")
                # Keep original palette
            
            if palette is not None:
                print(f"  -> Final Palette Size: {len(palette)} colors")            # Per-point colors for debug
...
def propagate_and_harvest(anchor_pts, palette, img_paths, depths, poses, intrs):
    ...
    for i in tqdm(range(len(img_paths))):
        ...
        # --- DEPTH BOUNDS ---
        ghost_depths = Z[valid_z]
        min_obj_z = np.percentile(ghost_depths, 5) 
        max_obj_z = np.percentile(ghost_depths, 95)
        ...
        # --- CREATE GEOMETRIC MASK (Hull Spotlight) ---
        ...
        # DILATION: Safe to use aggressive dilation now because of Background Learning
        ...
        # --- FILTERS ---
        # 1. Depth Tolerance (Relaxed)
        tolerance = 0.5 
        valid_depth = (z_new > 0.1) & \
                      (z_new > min_obj_z - tolerance) & \
                      (z_new < max_obj_z + tolerance)
                      
        # 2. COLOR PALETTE GUARD
        if palette is not None:
            # Distance to nearest VALID object color
